...
impl FeatureStore {
    ...
    pub fn load_daily_bars(
        ...
        let rows = stmt
            .query_map(
                params![symbol.as_str(), &start_str, &start_str, &end_str, &end_str,],
                |row| {
                    Ok((
                        row.get::<_, String>(0)?,
                        row.get::<_, f64>(1)?,
        ...
        for row_result in rows {
            let (ts, open, high, low, close, volume) =
                row_result.map_err(|e| MlError::Store(e.to_string()))?;
    ...
    pub async fn export_features_parquet(
        ...
        let mut df = DataFrame::new(vec![
            ...
            Series::new(
                "macd_signal",
                rows.iter().map(|row| row.macd_signal).collect::<Vec<_>>(),
            ),
            Series::new(
                "bb_upper",
                rows.iter().map(|row| row.bb_upper).collect::<Vec<_>>(),
            ),
            Series::new(
                "bb_lower",
                rows.iter().map(|row| row.bb_lower).collect::<Vec<_>>(),
            ),
            Series::new("atr", rows.iter().map(|row| row.atr).collect::<Vec<_>>()),
            Series::new(
                "return_1d",
                rows.iter().map(|row| row.return_1d).collect::<Vec<_>>(),
            ),
            Series::new(
                "return_5d",
                rows.iter().map(|row| row.return_5d).collect::<Vec<_>>(),
            ),
            Series::new(
                "return_20d",
                rows.iter().map(|row| row.return_20d).collect::<Vec<_>>(),
            ),
            Series::new(
                "rolling_mean_20",
                rows.iter()
                    .map(|row| row.rolling_mean_20)
                    .collect::<Vec<_>>(),
            ),
            Series::new(
                "rolling_std_20",
                rows.iter()
                    .map(|row| row.rolling_std_20)
                    .collect::<Vec<_>>(),
            ),
            Series::new(
                "lag_1",
                rows.iter().map(|row| row.lag_1).collect::<Vec<_>>(),
            ),
            Series::new(
                "lag_2",
                rows.iter().map(|row| row.lag_2).collect::<Vec<_>>(),
            ),
            Series::new(
                "lag_3",
                rows.iter().map(|row| row.lag_3).collect::<Vec<_>>(),
            ),
            Series::new(
                "rolling_momentum",
                rows.iter()
                    .map(|row| row.rolling_momentum)
                    .collect::<Vec<_>>(),
            ),
        ])?;
